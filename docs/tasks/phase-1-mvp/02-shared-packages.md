...
- [ ] shared-types 套件可被前後端專案引用
- [ ] 所有型別定義完整且無 `any` 型別（測試除外）
...
- [ ] 建立 `packages/shared-types/package.json`
  ```json
  {
...
- [ ] `enums/order.ts`（PaymentMethod, PaymentStatus, ShippingMethod 等）
- [ ] `enums/auction.ts`（AuctionStatus）
- [ ] `enums/common.ts`（UserRole, AdminRole 等）
...
#### 2.1.8 建立文檔
- [ ] 建立 `packages/shared-types/README.md`
  - 套件說明
  - 安裝與使用方式
  - 型別範例
