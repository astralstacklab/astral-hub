...
Card ERP 共用工具函數庫，提供格式化、驗證、計算與字串處理等常用純函數。
...
```ts
import { formatCurrency } from '@card-erp/utils'
...
```ts
import { isValidEmail } from '@card-erp/utils'
...
```ts
import { calculateNetIncome } from '@card-erp/utils'
...
```ts
import { toSlug, sanitizeHtml, truncate } from '@card-erp/utils'
...
# 測試（監看）
pnpm --filter @card-erp/utils test:watch

# 測試覆蓋率
pnpm --filter @card-erp/utils test:coverage
